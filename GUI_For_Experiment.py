...
##################################################################################
        self.pushButton_7.clicked.connect(self.thread_4)
        self.pushButton.clicked.connect(self.thread_2)
##################################################################################      
...
###################Plots go here#######################################
        ...
        self.plot_3 = self.widget_12.plot(self.t,self.roll,pen=pen,fillLevel = 1,fillBrush = (255,255,255,50))
        self.plot_4 = self.widget_9.plot(self.t,self.pitch,pen=pen,fillLevel = 1,fillBrush = (255,255,255,50))
...
###################################################################################################
        ...
        if self.Lstartbutton:
            if len(self.firstRun) <= self.DataPoints:  
                ...
                if len(data) == 4:
                    ...
                    self.roll.append(float(data[2]))
                    self.pitch.append(float(data[3]))
                    self.pressure.append(float(data[0]))
    ...
    def UpdatePlot_2(self):
        if self.Lstartbutton:
        	self.plot_1.setData(self.t,self.pressure)
        	self.plot_2.setData(self.t,self.temperature)
        	self.plot_3.setData(self.t,self.roll)
        	self.plot_4.setData(self.t,self.pitch)

    def SURGE(self):
        if self.Lstartbutton and self.Start:
            ser.write((self.pwm_1[0]).encode())
    ...
    def CAPTURE_1(self):
        if not self.Start:
            self.pwm_1[0] = (self.lineEdit.text())
            ser.write((self.pwm_1[0]).encode())
            self.Start.append(1)
